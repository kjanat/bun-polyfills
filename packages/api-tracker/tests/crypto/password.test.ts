  ...
  describe("arguments parsing", () => {
    for (const hash of [password.hash, password.hashSync]) {
      ...
      test("invalid algorithm throws", () => {
        ...
        expect(() =>
          hash(placeholder, { algorithm: "bcrypt", cost: Infinity }),
        ).toThrow();

        expect(() =>
          hash(placeholder, { algorithm: "argon2id", memoryCost: -1 }),
        ).toThrow();

        expect(() =>
          hash(placeholder, { algorithm: "argon2id", timeCost: -1 }),
        ).toThrow();

        expect(() =>
          hash(placeholder, { algorithm: "bcrypt", cost: -999 }),
        ).toThrow();
      });
    ...
    for (const verify of [password.verify, password.verifySync]) {
      ...
      for (const ArrayBufferView of [
        ...
        test(`empty ${ArrayBufferView.name} returns false`, async () => {
          expect(
            await verify(new ArrayBufferView(0), new ArrayBufferView(0)),
          ).toBeFalse();
      ...
      describe(typeof input === "string" ? "string" : "buffer", () => {
        ...
        describe("password", async () => {
          ...
          async function runSlowTestWithOptions(algorithmLabel: any) {
            ...
            expect(
              await password.verify(input, hashed, algorithmLabel),
            ).toBeTrue();
            expect(() =>
