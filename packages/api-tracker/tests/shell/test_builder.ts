...
export function createTestBuilder(path: string) {
   ...
   */
     ...
     */
    testMini(opts?: {
      errorOnSupportedTemplate?: boolean;
      onlyMini?: boolean;
      cwd?: string;
    }): this {
      this._testMini = true;
     ...
     */
    stdout(
      expected: string | ((stdout: string, tempDir: string) => void),
    ): this {
    ...
    stderr(
      expected: string | ((stderr: string, tempDir: string) => void),
    ): this {
    ...
    fileEquals(
      filename: string,
      expected: string | (() => string | Promise<string>),
    ): this {
      this.getTempDir();
    ...
    async doChecks(
      ...
      if (this.expected_stderr !== undefined) {
        if (typeof this.expected_stderr === "string") {
          expect(stderr.toString()).toEqual(
            this.expected_stderr.replaceAll("$TEMP_DIR", tempdir),
          );
      ...
      } else if (typeof this.expected_exit_code === "function")
        this.expected_exit_code(exitCode);
    ...
    async run(): Promise<undefined> {
      if (!insideTestScope) {
        const err = new Error(
          "TestBuilder.run() must be called inside a test scope",
        );
      ...
      try {
        ...
        if (typeof this.expected_error === "string") {
          ...
          expect(e.stdout.toString()).toEqual(
            this.expected_error.stdout.toString(),
          );
          expect(e.stderr.toString()).toEqual(
            this.expected_error.stderr.toString(),
          );
        }
        return undefined;
      }
