...
afterAll(() =>
  console.error("After all RSS", process.memoryUsage.rss() / 1024 / 1024),
);
beforeAll(() =>
...
const BUN = nodeWithPolyfillsExe();
...
describe("bunshell", () => {
  ...
  describe("escape", async () => {
    ...
    describe("wrapped in quotes", async () => {
      ...
      TestBuilder.command`echo url="${url}"`
        .stdout(`url=${url}\n`)
    ...
    describe("escape var", async () => {
      const shellvar = "$FOO";
      TestBuilder.command`FOO=bar && echo "${shellvar}"`
        .stdout(`$FOO\n`)
        .runAsTest("double quotes");
      TestBuilder.command`FOO=bar && echo '${shellvar}'`
        .stdout(`$FOO\n`)
    ...
    test("can't escape a js string/obj ref", async () => {
      ...
      expect(async () => {
        await TestBuilder.command`echo hi > \\${buf}`
          .error("Redirection with no file")
          .run();
      });
    });
  ...
  describe("quiet", async () => {
    ...
    test.each([
      ...
      "$description suppresses output: $expectedQuiet",
      async ({ value, expectedQuiet }) => {
...
`.run();
  ...
  describe("echo+cmdsubst edgecases", async () => {
    ...
    describe("word splitting", async () => {
      TestBuilder.command`echo $(echo id)/$(echo region)`
        .stdout("id/region\n")
  ...
  describe("operators no spaces", async () => {
    ...
    TestBuilder.command`echo foo||echo hi`.stdout("foo\n").runAsTest("||");
    TestBuilder.command`echo foo>hi.txt`
      .ensureTempDir()
      .fileEquals("hi.txt", "foo\n")
      .runAsTest("||");
    TestBuilder.command`echo hifriends#lol`
      .stdout("hifriends#lol\n")
      .runAsTest("#");
  });
  ...
  describe("empty_expansion", () => {
    TestBuilder.command`$(exit 0) && echo hi`
      .stdout("hi\n")
      .runAsTest("empty command subst");
    TestBuilder.command`$(exit 1) && echo hi`
      .exitCode(1)
      .runAsTest("empty command subst 2");
    TestBuilder.command`FOO="" $FOO`.runAsTest("empty var");
  });

  describe("tilde_expansion", () => {
    describe("with paths", async () => {
      TestBuilder.command`echo ~/Documents`
        .stdout(`${process.env.HOME}/Documents\n`)
        .runAsTest("normal");
      TestBuilder.command`echo ~/Do"cu"me"nts"`
        .stdout(`${process.env.HOME}/Documents\n`)
        .runAsTest("compound word");
      TestBuilder.command`echo ~/LOL hi hello`
    ...
    describe("normal", async () => {
      ...
      TestBuilder.command`echo ~ hi hello`
    ...
    describe("modified $HOME or $USERPROFILE", async () => {
      TestBuilder.command`HOME=lmao USERPROFILE=lmao && echo ~`
        .stdout("lmao\n")
        .runAsTest("1");
...
bar\n`,
  ...
  `
      ...
      .stdout((out) =>
        expect(JSON.parse(out).slice(2)).toEqual([
          "hi",
          "hello",
          "on",
          "a",
...
booga"
  ...
  describe("glob expansion", () => {
    ...
    TestBuilder.command`touch hihello; touch hifriends; FOO=hi*; echo $FOO`
      .ensureTempDir()
      .stdout((s) =>
        expect(s).toBeOneOf(["hihello hifriends\n", "hifriends hihello\n"]),
      )
...
${temp_dir}`
  ...
  describe("escaping", () => {
    ...
    TestBuilder.command`echo ${"\\"}`.stdout("\\\n").runAsTest("backslash");
    TestBuilder.command`echo ${" "}`.stdout(" \n").runAsTest("space");
    TestBuilder.command`echo ${"'hello'"}`
      .stdout("'hello'\n")
    ...
    TestBuilder.command`echo ${"hello; echo world"}`
      .stdout("hello; echo world\n")
      .runAsTest("compound_command");
    TestBuilder.command`echo ${"hello > world"}`
      .stdout("hello > world\n")
      .runAsTest("redirect_in_echo");
    TestBuilder.command`echo ${"$(echo nested)"}`
      .stdout("$(echo nested)\n")
...
describe("deno_task", () => {
  describe("commands", async () => {
    TestBuilder.command`echo 1`.stdout("1\n").runAsTest("echo 1");
    TestBuilder.command`echo 1 2   3`
      .stdout("1 2 3\n")
    ...
    TestBuilder.command`echo "1 2\ \ \ 3"`
      .stdout("1 2\\ \\ \\ 3\n")
      .runAsTest('echo "1 2\\ \\ \\ 3"');
    TestBuilder.command`echo test$(echo 1    2)`
      .stdout("test1 2\n")
    ...
    TestBuilder.command`echo "test$(echo "1    2")"`
      .stdout("test1    2\n")
      .runAsTest('echo "test$(echo "1    2")"');
    TestBuilder.command`echo test$(echo "1 2 3")`
      .stdout("test1 2 3\n")
    ...
    TestBuilder.command`echo test-dashes`
      .stdout("test-dashes\n")
      .runAsTest("echo test-dashes");
    TestBuilder.command`echo 'a/b'/c`
      .stdout("a/b/c\n")
  ...
  describe("command substitution", async () => {
    TestBuilder.command`echo $(echo 1)`
      .stdout("1\n")
      .runAsTest("nested echo cmd subst");
    TestBuilder.command`echo $(echo 1 && echo 2)`
      .stdout("1 2\n")
  ...
  describe("shell variables", async () => {
    ...
    TestBuilder.command`VAR=1 && echo $VAR$VAR`
      .stdout("11\n")
  ...
  describe("pipeline", async () => {
    ...
    if (isPosix) {
      TestBuilder.command`ls . | echo hi`
        .exitCode(0)
        .stdout("hi\n")
    ...
    TestBuilder.command`echo hi | ${BUN} -e 'process.exit(69)'`
      .exitCode(69)
      .stdout("")
      .runAsTest("last exit code 3");
  ...
  describe("redirects", async function igodf() {
    TestBuilder.command`echo 5 6 7 > test.txt`
      .fileEquals("test.txt", "5 6 7\n")
    ...
    TestBuilder.command`echo hi > /dev/null`
      .quiet()
      .runAsTest("redirect /dev/null");
  ...
  test("big_data", async () => {
    ...
    const { stdout, stderr, exitCode } =
      await $`${BUN} -e ${writerCode} > ${tmpdir}/output.txt`.env(bunEnv);
...
`,
  ...
  then
    ...
    .stdout("okay here\n")
    .stderr(
      "bun: command not found: lkfjslkdjfsldf\nbun: command not found: sdfkjsldf\n",
    )
    .runAsTest("else");
  ...
  TestBuilder.command`echo fif hi`
    .stdout("fif hi\n")
  ...
  describe("ported from posix shell tests", () => {
    ...
    TestBuilder.command`if ${exit(0)}; then ${exit(0)}; fi`
      .exitCode(0)
      .runAsTest("exit status of if, true-true");
    // test_x -e 1 'exit status of if, true-false'
    TestBuilder.command`if ${exit(0)}; then ${exit(1)}; fi`
      .exitCode(1)
      .runAsTest("exit status of if, true-false");

    // test_x -e 0 'exit status of if, false'
    TestBuilder.command`if ${exit(1)}; then ${exit(2)}; fi`
      .exitCode(0)
      .runAsTest("exit status of if, false");
...
describe("condexprs", () => {
  TestBuilder.command`[[ -f package.json ]] && echo yes!`
    .file("package.json", "hi")
    .stdout("yes!\n")
    .runAsTest("-f");
  TestBuilder.command`[[ -f mumbo.jumbo ]] && echo yes!`
    .exitCode(1)
    .runAsTest("-f non-existent");

  TestBuilder.command`[[ -d mydir ]] && echo yes!`
    .directory("mydir")
    .stdout("yes!\n")
    .runAsTest("-d");
  TestBuilder.command`[[ -d mumbo.jumbo ]] && echo yes!`
    .exitCode(1)
    .runAsTest("-d non-existent");

  TestBuilder.command`[[ -c /dev/null ]] && echo yes!`
    .stdout("yes!\n")
    .runAsTest("-c");
  TestBuilder.command`[[ -c lol ]] && echo yes!`
    .exitCode(1)
    .file("lol", "lol")
    .runAsTest("-c not character device");
  TestBuilder.command`[[ -c mumbo.jumbo ]] && echo yes!`
    .exitCode(1)
    .runAsTest("-c non-existent");

  TestBuilder.command`FOO=""; [[ -z $FOO ]] && echo yes!`
    .stdout("yes!\n")
    .runAsTest("-z");
  TestBuilder.command`[[ -z "skldjfldsf" ]] && echo yes!`
    .exitCode(1)
    .runAsTest("-z fail");

  TestBuilder.command`FOO="lkjdflskdjf"; [[ -n $FOO ]] && echo yes!`
    .stdout("yes!\n")
    .runAsTest("-n");
  TestBuilder.command`FOO="" [[ -n $FOO ]] && echo yes!`
    .exitCode(1)
    .runAsTest("-n fail");

  TestBuilder.command`[[ -n hey ]] | echo hi | cat`
    .stdout("hi\n")
    .runAsTest("precedence: pipeline");

  TestBuilder.command`[[ foo == foo ]] && echo yes!`
    .stdout("yes!\n")
    .runAsTest("==");
  TestBuilder.command`[[ foo == lol ]] && echo yes!`
    .exitCode(1)
    .runAsTest("== fail");
  TestBuilder.command`[[ foo != foo ]] && echo yes!`
    .exitCode(1)
    .runAsTest("!= fail");
  TestBuilder.command`[[ lmao != foo ]] && echo yes!`
    .stdout("yes!\n")
    .runAsTest("!=");

  TestBuilder.command`LOL=; [[ $LOl == $LOL ]] && echo yes!`
    .stdout("yes!\n")
    .runAsTest("== empty");
  TestBuilder.command`LOL=; [[ $LOl != $LOL ]] && echo yes!`
    .exitCode(1)
...
`
    ...
    TestBuilder.command`[[ a ]]`
      .exitCode(0)
      .runAsTest("parenthesized terms didn't work right until post-2.04");
    TestBuilder.command`[[ (a) ]]`
      .exitCode(0)
      .runAsTest("parenthesized terms didn't work right until post-2.04");
    TestBuilder.command`[[ -n a ]]`
      .exitCode(0)
      .runAsTest("parenthesized terms didn't work right until post-2.04");
    TestBuilder.command`[[ (-n a) ]]`
      .exitCode(0)
    ...
    TestBuilder.command`[[ -n $UNSET ]]`
      .exitCode(1)
      .runAsTest("unset variables don't need to be quoted");
    TestBuilder.command`[[ -z $UNSET ]]`
      .exitCode(0)
      .runAsTest("unset variables don't need to be quoted");
...
describe("subshell", () => {
  ...
  TestBuilder.command /* sh */ `(exit 42); echo hi`
    .exitCode(0)
    .stdout("hi\n")
  ...
  TestBuilder.command`\(echo hi \)`
    .stderr("bun: command not found: (echo\n")
    .exitCode(1)
    .runAsTest("escaped subshell");
  TestBuilder.command`echo \\\(hi\\\)`
    .stdout("\\(hi\\)\n")
