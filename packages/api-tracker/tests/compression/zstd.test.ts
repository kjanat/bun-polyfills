...
describe("Zstandard compression", async () => {
  ...
  it("throws with invalid level", () => {
    expect(() =>
      zstdCompressSync(new Uint8Array(123), { level: 0 }),
    ).toThrowErrorMatchingInlineSnapshot(
      `"Compression level must be between 1 and 22"`,
    );
    expect(() =>
      zstdCompress(new Uint8Array(123), { level: 0 }),
    ).toThrowErrorMatchingInlineSnapshot(
      `"Compression level must be between 1 and 22"`,
    );
  });
          ...
          JSON.stringify(
            {
              ...
              scripts: {
                ...
                "build:smol":
                  "bun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=MinSizeRel -B build/release-smol",
                "build:local":
                ...
                "css-properties":
                  "bun run src/css/properties/generate_properties.ts",
                "uv-posix-stubs":
                  "bun run src/bun.js/bindings/libuv/generate_uv_posix_stubs.ts",
                ...
                "test:release":
                  "node scripts/runner.node.mjs --exec-path ./build/release/bun",
                banned: "bun test test/internal/ban-words.test.ts",
                "glob-sources": "bun scripts/glob-sources.mjs",
                zig: "vendor/zig/zig.exe",
                "zig:test":
                  "bun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=Debug -DBUN_TEST=ON -B build/debug",
                "zig:test:release":
                ...
                "zig:check-windows":
                  "bun run zig build check-windows --summary new",
                analysis:
                ...
                "clang-format:check":
                  "bun run analysis --target clang-format-check",
                "clang-format:diff":
                  "bun run analysis --target clang-format-diff",
                "clang-tidy": "bun run analysis --target clang-tidy",
                "clang-tidy:check":
                  "bun run analysis --target clang-tidy-check",
                "clang-tidy:diff": "bun run analysis --target clang-tidy-diff",
                "zig-format": "bun run analysis:no-llvm --target zig-format",
                "zig-format:check":
                  "bun run analysis:no-llvm --target zig-format-check",
                prettier:
                ...
                "node:test":
                  "node ./scripts/runner.node.mjs --quiet --exec-path=$npm_execpath --node-tests ",
                "clean:zig":
...
describe("Zstandard HTTP compression", () => {
  ...
  const testData = {
    ...
    json: {
      ...
      name: "Test Object",
      values: [1, 2, 3, 4, 5],
      nested: { prop1: "value1", prop2: "value2" },
    },
    binary: Buffer.from(
    ...
    server = Bun.serve({
      ...
      async fetch(req) {
        ...
        // Route: /binary
          ...
          if (supportsZstd) {
            const compressed = await zstdCompress(testData.binary, {
              level: 3,
            });
            return new Response(compressed, {
              headers: {
        ...
        // Route: /echo
          ...
          if (supportsZstd) {
            const compressed = await zstdCompress(new Uint8Array(body), {
              level: 3,
            });
            return new Response(compressed, {
              headers: {
  ...
  it("can POST and receive zstd-compressed echo response", async () => {
    ...
    const response = await fetch(`${serverBaseUrl}/echo`, {
      method: "POST",
      headers: { "Content-Type": "text/plain", "Accept-Encoding": "zstd" },
      body: testString,
    });
