...
describe("stringWidth extended", () => {
  ...
  describe("OSC sequences", () => {
    test("OSC 8 hyperlinks with BEL terminator", () => {
      expect(
        Bun.stringWidth("\x1b]8;;https://example.com\x07link\x1b]8;;\x07"),
      ).toBe(4);
      expect(
        Bun.stringWidth("before\x1b]8;;url\x07click\x1b]8;;\x07after"),
      ).toBe(16);
    });
    ...
    test("OSC 8 hyperlinks with ST terminator", () => {
      ...
      expect(
        Bun.stringWidth("\x1b]8;;https://example.com\x1b\\link\x1b]8;;\x1b\\"),
      ).toBe(4);
      // Multiple OSC sequences with ST
      expect(Bun.stringWidth("a\x1b]0;title\x1b\\b\x1b]0;title2\x1b\\c")).toBe(
        3,
      );
    });
    ...
    test("mixed OSC and CSI", () => {
      expect(
        Bun.stringWidth("\x1b[31m\x1b]8;;url\x07red link\x1b]8;;\x07\x1b[0m"),
      ).toBe(8);
    });
  });
  ...
  describe("fuzzer-like stress tests", () => {
    ...
    test("worst case: every character needs special handling", () => {
      ...
      const input =
        "a\x1b[31m中\u0300\uFE0F👨‍👩‍👧🇺🇸\x1b]8;;url\x07link\x1b]8;;\x07\u200B\x1b[0m".repeat(
          500,
        );
      expect(Bun.stringWidth(input)).toBeGreaterThan(0);
    });
