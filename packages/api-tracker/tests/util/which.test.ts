...

{
  ...
  if (`${delim}${process.env.PATH}${delim}`.includes(`${delim}.${delim}`)) {
    throw new Error(
      "$PATH includes . which will break `Bun.which` tests. This is an environment configuration issue.",
    );
  }
}
  ...
  test("which", () => {
    ...
    try {
      ...
      expect(
        ...
        which("myscript.sh", { PATH: basedir }),
      ).toBe(abs);
      ...
      expect(which("myscript.sh", { PATH: "/not-tmp" })).toBe(null);
    } finally {
