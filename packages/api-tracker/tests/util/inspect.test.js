...
describe("latin1 supplemental", () => {
  ...
  it(`latin1 (property key)`, () => {
    expect(Object.keys({ ä: 1 })[0].codePointAt(0)).toBe(228);
  });
});
...
it("new Date(..)", () => {
  ...
  expect(Bun.inspect(new Date(2023, 2, 27, 9 - offset, 54, 0))).toBe(
    "2023-03-27T09:54:00.000Z",
  );
...
it("ErrorEvent", () => {
  ...
  expect(
    normalizeBunSnapshot(Bun.inspect(errorEvent)).replace(
      /\d+ \| /gim,
      "NNN |",
    ),
