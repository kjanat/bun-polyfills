  ...
  describe(label, () => {
    describe("spawnSync", () => {
      ...
      it("check exit code", async () => {
        ...
        const { exitCode: exitCode2 } = spawnSync({
          cmd: [nodeWithPolyfillsExe(), "-e", "process.exit(1)"],
        });
        gcTick();
    ...
    describe("spawn", () => {
      ...
      it("as an array with options object", async () => {
        gcTick();
        const { stdout } = spawn({
          cmd: [nodeWithPolyfillsExe(), "-e", "console.log(process.env.FOO)"],
          cwd: tmp,
      ...
      it("nothing to stdout and sleeping doesn't keep process open 4ever", async () => {
        const proc = spawn({ cmd: [shellExe(), "-c", "sleep 0.1"] });
        gcTick();
      ...
      it.skip("Uint8Array works as stdout", () => {
        ...
        const { stdout } = spawnSync(
          ...
          { stdout: stdout_buffer, stderr: null, stdin: null },
        );
        gcTick();
        ...
        for (let i = 0; i < promises.length; i++) {
          const { stdout, exited } = spawn({
            cmd: [
              nodeWithPolyfillsExe(),
              "-e",
              `require('fs').createReadStream(${JSON.stringify(filePath)}).pipe(process.stdout)`,
            ],
            stdout: "pipe",
            stdin: "ignore",
            stderr: "inherit",
      ...
      it("stdin can be read and stdout can be written", async () => {
        const proc = spawn({
          cmd: [
            "node",
            "-e",
            "process.stdin.setRawMode?.(true); process.stdin.pipe(process.stdout)",
          ],
          stdout: "pipe",
          stdin: "pipe",
          lazy: true,
...
it.skipIf(
  Boolean(process.env.BUN_FEATURE_FLAG_FORCE_WAITER_THREAD) ||
    !isPosix ||
    isMacOS,
)(
  ...
  async () => {
    const result = spawnSync({
      cmd: [nodeWithPolyfillsExe(), "test", path.resolve(import.meta.path)],
      env: {
        ...bunEnv,
...
describe("spawn unref and kill should not hang", () => {
  ...
  it("should not hang after unref", async () => {
    const proc = spawn({
      cmd: [
        nodeWithPolyfillsExe(),
        path.join(import.meta.dir, "does-not-hang.js"),
      ],
    });
...
describe("onDisconnect", () => {
  ...
  it.todoIf(isWindows)(
    "onDisconnect callback is called when IPC disconnects",
    async () => {
  ...
  it("onDisconnect is not called when IPC is not used", async () => {
    await using proc = spawn({
      cmd: [nodeWithPolyfillsExe(), "-e", "console.log('hello')"],
      onDisconnect: () => {
  ...
  it("argv0 option changes process.argv0 but not executable", async () => {
    await using proc = spawn({
      cmd: [
        nodeWithPolyfillsExe(),
        "-e",
        "console.log(process.argv0); console.log(process.execPath)",
      ],
      argv0: "custom-argv0",
      stdout: "pipe",
      stderr: "ignore",
  ...
  it("argv0 option works with spawnSync", () => {
    ...
    const proc = spawnSync({
      cmd: [
        nodeWithPolyfillsExe(),
        "-e",
        "console.log(JSON.stringify({ argv0: process.argv0, execPath: process.execPath }))",
      ],
      argv0,
      stdout: "pipe",
      stderr: "ignore",
    ...
    expect(output).toEqual({
      argv0,
      execPath: path.normalize(nodeWithPolyfillsExe()),
    });
  });

  it("argv0 defaults to cmd[0] when not specified", async () => {
    await using proc = spawn({
      cmd: [nodeWithPolyfillsExe(), "-e", "console.log(process.argv0)"],
      stdout: "pipe",
      stderr: "ignore",
      stdin: "ignore",
      env: bunEnv,
    ...
    const output = await proc.stdout.text();
    expect(output.trim()).toBe(nodeWithPolyfillsExe());
    await proc.exited;
  });
});
...
describe("option combinations", () => {
  ...
  it.todoIf(isWindows)(
    "onDisconnect + ipc + serialization works together",
    async () => {
